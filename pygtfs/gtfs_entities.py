...
class Agency(Base):
	...
	# for trimet
	bikes_policy_url = Column(Unicode, nullable=True)
...
class Stop(Base):
	...
	# routes = relationship("Route", secondary=routestops)
	...
	_validate_wheelchair = _validate_int_choice([None,0,1,2], 'wheelchair_boarding')
...
class Trip(Base):
	...
	trip_type = Column(Unicode, nullable=True)
	...
	_validate_wheelchair = _validate_int_choice([None,0,1,2], 'wheelchair_accessible')
...
class StopTime(Base):
	...
	# for trimet (a great idea!)
	timepoint = Column(Unicode, nullable=True)
	continuous_stops = Column(Unicode, nullable=True)
